...
static bool trace_read(GB_gameboy_t *gb, char *arguments, char *modifiers, const debugger_command_t *command)
{
    ...
    if(gb->memory_trace_read_enabled) {
        GB_log(gb, "Memory Read Tracer ENABLED (BROLZYO)\n");
    } else {
        GB_log(gb, "Memory Read Tracer DISABLED (BROLZYO)\n");
    }
    return true;
}
static bool trace_write(GB_gameboy_t *gb, char *arguments, char *modifiers, const debugger_command_t *command)
{
    ...
    if(gb->memory_trace_write_enabled) {
        GB_log(gb, "Memory Write Tracer ENABLED (BROLZYO)\n");
    } else {
        GB_log(gb, "Memory Write Tracer DISABLED (BROLZYO)\n");
    }
    return true;
}
static bool trace_all(GB_gameboy_t *gb, char *arguments, char *modifiers, const debugger_command_t *command)
{
    gb->memory_trace_read_enabled = !gb->memory_trace_read_enabled;
    gb->memory_trace_write_enabled = !gb->memory_trace_write_enabled;
    if(gb->memory_trace_read_enabled && gb->memory_trace_write_enabled) {
        GB_log(gb, "ALL Memory Tracer ENABLED (BROLZYO)\n");
    } else {
        GB_log(gb, "ALL Memory Tracer DISABLED (BROLZYO)\n");
    }
    return true;
}
static bool trace_dump(GB_gameboy_t *gb, char *arguments, char *modifiers, const debugger_command_t *command)
{
    FILE *write_file = fopen(arguments, "wb");
    fwrite(gb->memory_trace, sizeof(gb->memory_trace), 1, write_file);
    GB_log(gb, "DUMP File \"%s\"\n", arguments);
    //fclose(write_file);
    return true;
}
...
static const debugger_command_t commands[] = {
    ...
    {"trace_read", 1, trace_read, "Trace Memory Reads"},
    {"trace_write", 1, trace_write, "Trace Memory Writes"},
    {"trace_all", 1, trace_all, "Trace All Memory"},
    {"trace_clear", 1, trace_clear, "Clear Memory Trace"},
    {"trace_dump", 1, trace_dump, "Dump Memory Trace"},
